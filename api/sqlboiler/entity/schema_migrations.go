...
func (o *SchemaMigration) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to populate default values for schema_migrations")
	}
...
func (o *SchemaMigration) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to upsert for schema_migrations")
	}
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to retrieve unique values for schema_migrations")
	}
...
func (o SchemaMigrationSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	ln := int64(len(o))
	if ln == 0 {
		return nil
	}
	...
	for i, row := range o {
		...
		if i == 0 {
			sql = "INSERT INTO `schema_migrations` " + "(`" + strings.Join(insert, "`,`") + "`)" + " VALUES "
		}
		sql += strmangle.Placeholders(dialect.UseIndexPlaceholders, len(insert), len(vals)+1, len(insert))
		if i != len(o)-1 {
			sql += ", "
		}
		valMapping, err := queries.BindMapping(schemaMigrationType, schemaMigrationMapping, insert)
		if err != nil {
			return err
		}
		value := reflect.Indirect(reflect.ValueOf(row))
