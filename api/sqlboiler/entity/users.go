...
func (o UserSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	ln := int64(len(o))
	if ln == 0 {
		return nil
	}
	...
	for i, row := range o {
		...
		if i == 0 {
			sql = "INSERT INTO `users` " + "(`" + strings.Join(insert, "`,`") + "`)" + " VALUES "
		}
		sql += strmangle.Placeholders(dialect.UseIndexPlaceholders, len(insert), len(vals)+1, len(insert))
		if i != len(o)-1 {
			sql += ", "
		}
		valMapping, err := queries.BindMapping(userType, userMapping, insert)
		if err != nil {
			return err
		}
		value := reflect.Indirect(reflect.ValueOf(row))
