...
func (r *postR) GetReplies() ReplySlice {
	if r == nil {
		return nil
	}
	return r.Replies
...
var (
	...
	postColumnsWithoutDefault = []string{"id", "user_id", "content", "created_at"}
	postColumnsWithDefault    = []string{"updated_at"}
...
func (o *Post) Replies(mods ...qm.QueryMod) replyQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
func (postL) LoadLikes(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`likes`),
		qm.WhereIn(`likes.post_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (postL) LoadReplies(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	var slice []*Post
	var object *Post
	...
	query := NewQuery(
		qm.From(`replies`),
		qm.WhereIn(`replies.post_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Post appropriately.
func (o *Post) AddReplies(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Reply) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.PostID = o.ID
...
func (o *Post) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to populate default values for posts")
	}
...
func (o *Post) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to upsert for posts")
	}
	...
	if err != nil {
		return errors.Wrap(err, "sqlboiler: unable to retrieve unique values for posts")
	}
...
func (o PostSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	ln := int64(len(o))
	if ln == 0 {
		return nil
	}
...
func (o PostSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	ln := int64(len(o))
	if ln == 0 {
		return nil
	}
	...
	for i, row := range o {
		...
		if i == 0 {
			sql = "INSERT INTO `posts` " + "(`" + strings.Join(insert, "`,`") + "`)" + " VALUES "
		}
		sql += strmangle.Placeholders(dialect.UseIndexPlaceholders, len(insert), len(vals)+1, len(insert))
		if i != len(o)-1 {
			sql += ", "
		}
		valMapping, err := queries.BindMapping(postType, postMapping, insert)
		if err != nil {
			return err
		}
		value := reflect.Indirect(reflect.ValueOf(row))
